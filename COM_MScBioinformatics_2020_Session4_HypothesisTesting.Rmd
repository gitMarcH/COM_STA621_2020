...
* Difference in odds rations or relative risks
...
```{r}
t.test(df$age,mu=24,alternative="less")
```
...
```{r}
t.test(df$cd41,df$cd42,paired = TRUE)
...
```{r}
res.hosp <- aov(cd41 ~ factor(hosp), data = df)
summary(res.hosp)
```
...
```{r}
# create variable
df$agecat <- ifelse(df$age <=30,1,2)
wilcox.test(cd41.sk ~ agecat,data = df)
...
```{r}
wilcox.test(df$cd41.sk,df$cd42.sk,paired = TRUE)
...
* P-values do not measure the probability of $H_0$ being true.
